...
export default function UsuariosPage() {
  ...
  const fetchCustomers = async () => {
    ...
    try {
      ...
      setError("Error al cargar clientes desde Medusa");
    } finally {
      setLoading(false);
    }
  };
  ...
  const stats = useMemo(() => {
    ...
    return {
      ...
      guests: customers.filter((c) => !c.has_account).length,
  ...
  const statCards = [
    ...
    { label: "Con Cuenta", value: stats.withAccount, icon: UserCheck, color: "text-emerald-400" },
    { label: "Invitados", value: stats.guests, icon: UserX, color: "text-zinc-400" },
    ...
    <div className="space-y-6">
      ...
      >
        {statCards.map((stat) => (
          <div
            key={stat.label}
            className="rounded-xl border border-zinc-800 bg-zinc-900/80 backdrop-blur-sm p-4"
          >
      ...
      {loading ? (
        <div className={viewMode === "grid" ? "grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4" : "space-y-3"}>
          {Array.from({ length: 6 }).map((_, i) => (
            <div
              key={i}
              className={`animate-pulse rounded-xl border border-zinc-800 bg-zinc-900/60 ${
        ...
        >
          <div className="rounded-2xl bg-zinc-800/50 p-6 mb-4">
          ...
          <p className="text-sm text-zinc-500 mt-2 max-w-sm">
            {search
              ...
              : "Los clientes apareceran aqui cuando se registren o realicen compras."}
          </p>
        </motion.div>
      ) : (
        /* Customer Grid / List */
            ...
            {filtered.map((c) => {
              ...
              if (viewMode === "list") {
                return (
                  ...
                  >
                    <div
                      className={`flex size-10 shrink-0 items-center justify-center rounded-full bg-gradient-to-br ${avatarColor} text-white text-sm font-bold shadow-lg`}
                    >
                      {initials}
                    </div>
              ...
              return (
                ...
                >
                  ...
                  <div className="relative flex flex-col items-center text-center">
                    {/* Avatar */}
                    <div
                      className={`flex size-14 items-center justify-center rounded-full bg-gradient-to-br ${avatarColor} text-white text-lg font-bold shadow-lg ring-2 ring-zinc-800 group-hover:ring-zinc-700 transition-all`}
                    >
                      {initials}
                    </div>

                    {/* Name */}
                    <h3 className="mt-3 text-sm font-semibold text-zinc-100 truncate w-full">
                      {name}
                    </h3>
