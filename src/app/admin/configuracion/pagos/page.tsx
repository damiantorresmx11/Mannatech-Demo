...
import { getPaymentMethods } from "@/lib/commerce/client";
...
export default function PagosPage() {
  ...
  const fetch_ = async () => {
    ...
    try { const data = await getPaymentMethods(); setProviders(data || []); }
    ...
    <div className="space-y-6">
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center py-20 text-center">
          ...
          <p className="text-sm text-zinc-500 mt-1">Configura proveedores de pago desde BigCommerce.</p>
        </div>
      ) : (
              ...
              <CardContent className="pt-1">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h3 className="font-medium text-zinc-900 dark:text-zinc-100 capitalize">{p.nombre}</h3>
                      <p className="text-xs text-zinc-500 font-mono">{p.tipo}</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-2">
                    <div className={`size-3 rounded-full ${p.activo ? "bg-emerald-500" : "bg-zinc-300 dark:bg-zinc-600"}`} />
                    <span className="text-sm text-zinc-600 dark:text-zinc-400">{p.activo ? "Activo" : "Inactivo"}</span>
                  </div>
                </div>
              </CardContent>
            </Card>
