...
export default function LoginSocioPage() {
  ...
  function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    // Demo only — redirect to associate dashboard
    router.push("/socio/resumen");
  }
    ...
    <div className="min-h-[70vh] flex items-center justify-center bg-muted/30 dark:bg-zinc-950 py-16 px-4">
      <div className="w-full max-w-md bg-white dark:bg-zinc-900 rounded-2xl shadow-lg p-8 sm:p-10">
        <div className="text-center mb-8">
          ...
          <p className="text-sm text-muted-foreground mt-2">Socio / Asociado Mannatech</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <button
            type="submit"
            className="w-full bg-mannatech hover:bg-mannatech-dark text-white font-semibold py-3.5 rounded-lg transition-colors text-sm uppercase tracking-wider"
          >
            Iniciar Sesion
          </button>
        </form>
        ...
        <div className="text-center mt-6 space-y-3">
          ...
          <div className="pt-4 border-t border-border/50">
            <p className="text-xs text-muted-foreground">
              Accede a tu oficina virtual, panel de informes, genealogia y herramientas de negocio.
            </p>
          </div>
        </div>
      </div>
